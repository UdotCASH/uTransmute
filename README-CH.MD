...
ETHxEOS协议可以实现ETH和EOS跨链代币转移。

- .ETH (ERC20) –\&gt; ETHxEOS –\&gt; EOS (代币)

本协议的目标是为app开发人员提供一个基础标准，来实现跨链操作代币和app。

尽请加入Join the main U.CASH Telegram group(https://t.me/ucash) 共同讨论ETHxEOS。同时也诚挚欢迎您为udotcash.github.io/ETHxEOS节点投票。
...
在ETHxEOS协议里，我们为没有内置暂停/到期功能且不能满足想要跨链转移代币的用户的ERC20合约提供了另一种方式。我们将此需求称为：teleportation隐形转移。链间转移一个代币，此代币将存在于目的链，但在源链上不再以可替换形式存在。
...
标准的SwapTunnel合约有2个功能——授权从Ethereum接收Y个代币，然后接收EOS账户信息，通过Oracle在目的链上分配代币。
...
- . **ETHxEOS/ETHxEOS.js**  - Oracle 负责管理代币从ETH到EOS转移。
- . **ETHxEOS/** [**config.json**](https://github.com/UdotCASH/ETHxEOS/blob/master/config.json) - swaptunnel和Oracle合约的配置文件
- . **ETHxEOS/swaptunnel/**** 合约 ****/**  - swaptunnel合约如下
  ...
  - . **sol**  - 确认EOS账户或钥匙
- . **ETHxEOS/swaptunnel/migrations/**  - 部署truffle测试的脚本
- . **ETHxEOS/swaptunnel/test/**  -swaptunnel的 truffle 测试
- . **ETHxEOS/eosio.token/**  - 来自EOS.IO GitHub的标准EOSIO代币合约测试
- . **ETHxEOS/utils/**  - 错误检查脚本
- . **ETHxEOS/package.json**  - 测试套件的NPM安装包

# 贡献

欢迎阅读 [CONTRIBUTING.md](https://udotcash.github.io/ETHxEOS/CONTRIBUTING.md)

_ETHxEOS is open-source and we encourage you to customize, fork, and use the code. We built this as a__n_ _example case. Some of the ideas we have include:_

_ETHxEOS代码是开源的，我们鼓励所有人定制、派生和使用代码。我们建立它是作为一个基石。以下是我们的一些想法：_

- ._可以修改ETHxEOS合约，使用注册EOS账户或钥匙来驱动快照分配。_
- ._可以编写ETHxEOS&quot; __teleporter__&quot;或 __&quot;__ racle&quot;，从而完全在EOS链上运行（代替了 __js__ ），并且简化的支付认证过程（SPV）也可完全在链上进行。_
- ._可以修改ETHxEOS合约，在Oracle成功将ETH代币转移至EOS后，通过将此部分ETH代币发送到0x00地址来完成代币燃烧。_
- ._可以修改ETHxEOS合约，允许代币在传送点ETH_ _↔ __EOS间传送，通过使用__&quot; __2-通道__&quot;__代币，锁定代币不再是靠每条链单独的合约。_
- ._ETHxEOS 可以在共享相同私匙的任何一个链上创建公匙。_
- ._ETHxEOS__可以用于验证EOS与ETH交易。_
- ._ETHxEOS 可以用于在EOS __子链__ 间转移代币。_
- ._可以改写ETHxEOS SwapTunnel合约，用于支持其它的Ethereum分支链，例如GoChain，和其他支持代币类似于 __Stellar__ 的链。_
...
1. **Create token on Ethereum.**** 在Ethereum上创建代币。** _Truffle_ _进行此操作。（4个代币将表示为40000，Ethereum合约中有4个小数点。 __——__ 可在_[config.json](https://github.com/UdotCASH/ETHxEOS/blob/master/config.json)_完成配置__)._
2. **向新Ethereum账户分配新代币。** _Truffle_ _进行此操作。_
3. _ **部署swaptunnel合约。** __合约地址在truffle配置文件中自动更新。_
4. **在**** Jungle Testnet ****部署标准eosio.token合约。**
5. _ **通过eosio.token合约发行EOS代币。** __参数见_ [config.json](https://github.com/UdotCASH/ETHxEOS/blob/master/config.json)_中配置。_
...
- .[Ganache](https://truffleframework.com/ganache) - 点击本地Ethereum区块链。
  - ._Ganache __应该被配置在8545端口上进行本地运行。（您可能需要在__ Ganache__首选项设置或编辑_ [config.json](https://github.com/UdotCASH/ETHxEOS/blob/master/config.json)匹配端口_）_
...
- . \*\*测试合约\*\*
  - .truffle test
- .\*\*部署ERC20合约和在 [config.json](https://github.com/UdotCASH/ETHxEOS/blob/master/config.json)中定义swaptunnel合约。\*\*
  - .truffle migrate --reset --network ganache
  - ._进程还将把你新创建的ERC20代币发送到你在 __Ganache__ 界面的第一个账户。_

## 步骤2：部署Oracle

- .\*\*从ETHxEOS 项目的根文件开始oracle\*\*
...
- .在[config.json](https://github.com/UdotCASH/ETHxEOS/blob/master/config.json)配置swaptunnel 和 eosiotoken部分到您的代币参数
...
- .在Ethereum主网上安装ETHxEOS/swaptunnel/contracts/SwapTunnelEosAccount.sol orswaptunnel/contracts/SwapTunnelEosPublicKey.sol
- .开始Oracle
...
This project is licensed under the MIT License - see the [LICENSE.md](https://udotcash.github.io/ETHxEOS/LICENSE.md) file for details

本项目获得 MIT许可——查看详细细节[LICENSE.md](https://udotcash.github.io/ETHxEOS/LICENSE.md)

[**ETHxEOS**](https://github.com/UdotCASH/ETHxEOS) ** is maintained by ** [**udotcash.github.io-org**](https://github.com/udotcash.github.io-org) **.** This page was generated by [GitHub Pages](https://pages.github.com/).

[**ETHxEOS**](https://github.com/UdotCASH/ETHxEOS) ** **** 由**[**udotcash.github.io-org**](https://github.com/udotcash.github.io-org)**维护 ****.**
