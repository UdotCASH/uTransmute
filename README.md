...
uTransmute is a protocol to enable cross-chain ⛓ token transmuting between ETH and EOS.
...
* **uTransmute/uTransmute.js** - Oracle for managing transmute of tokens from ETH to EOS
* **uTransmute/[config.json](https://github.com/UdotCASH/uTransmute/blob/master/config.json)** - configuration file for utransmute and oracle contracts
* **uTransmute/utransmute/contracts/** - utransmute contracts listed below
    * **uTransmute.sol**	- utransmute contract that will attract ERC20 tokens
    * **uTransmuteEosAccount.sol**	- utransmute contract that takes an EOS Account as an input to activate a transmute
    * **uTransmuteEosPublicKey.sol** - takes an EOS public key as an input to activate a transmute (you will need to create  accounts using a modified oracle for account creation)
...
* **uTransmute/utransmute/migrations/** - scripts for deploying truffle test
* **uTransmute/utransmute/test/** - truffle tests of the utransmute
...
* *uTransmute "transmuter" or "oracle" could be written to run entirely on an EOS chain (instead of node.js) and simplified payment verification (SPV) could be done entirely on-chain.*
...
* *uTransmute could be modified to allow tokens to travel both ways in the Transmuteper ETH ↔ EOS by using a "2-way-peg" of tokens - locking the tokens inside of a contract on each chain.*
...
3. **Deploy utransmute contract.** *Contract address will automatically update in the truffle config file).*
...
7. **Source Ethereum account must send 2 actions.**
    * Authorize utransmute to transmute an amount of ERC20 tokens.
    * Send EOS account name to activate transmute.
...
## Step 1: Truffle Deployment of Ethereum Contracts (ERC20 token + uTransmute)
...
* ** Deploy ERC20 contract and the utransmute contract defined in [config.json](https://github.com/UdotCASH/uTransmute/blob/master/config.json)**
...
* ** Use uTransmute Contract**
    * `uTransmuteEosAccount.deployed().then(i => utransmute=i)`


* ** Send Approval to uTransmute to Attract Tokens**
...
* ** Transmute tokens by entering destination EOS account**
...
### Your test tokens have been transmuted!
...
* In [config.json](https://github.com/UdotCASH/uTransmute/blob/master/config.json) configure `utransmute` and `eosiotoken` sections to your token parameters.
    * `websocket_provider` will point to Ethereum node - on mainnet use `wss://mainnet.infura.io/ws`
    * `critic_block` will be the Ethereum block number that you want the utransmute contract to expire, set to 0 if it never expires.
...
* Install `uTransmute/utransmute/contracts/uTransmuteEosAccount.sol` or `utransmute/contracts/uTransmuteEosPublicKey.sol` on Ethereum mainnet
* Start the Oracle 🔮 uTransmute/oracle/TransmuteOracle.js
* From an Ethereum account containing tokens you want to transmute, authorize the utransmute to attract tokens from your account, then send your EOS account name to the contract "transmute" action to initiate the movement of tokens to EOS. *This process could be made really simple through very good UX/UI design for an interface.*
